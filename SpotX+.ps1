...
function Set-UpdateBlocking {
    ...
    try {
        ...
        if (-not (Test-Path $script:UPDATE_FOLDER)) {
            New-Item -ItemType Directory -Path $script:UPDATE_FOLDER -Force | Out-Null
        }

        $acl = Get-Acl $script:UPDATE_FOLDER

        $existingDeny = $acl.Access | Where-Object { $_.AccessControlType -eq 'Deny' }
        foreach ($rule in $existingDeny) {
        ...
        Set-Acl $script:UPDATE_FOLDER $acl

        Write-Success "Spotify auto-updates blocked"
    }
    catch {
        Write-Fail "Could not block updates: $($_.Exception.Message)"
        Write-Info "You can block updates manually by setting the Update folder to read-only."
    }
}
...
function Invoke-BinaryPatching {
    ...
    # Step 1: Disable signature verification in Spotify.dll
    try {
    ...
    # Step 2: Remove Authenticode signatures from binaries
    try {
    ...
    # Step 3: Block ad slots at binary level (v1.2.94+)
    ...
    # Step 4: Enable crossfade for free users (v1.2.89+)
...
function Invoke-SpotXPlus {
    ...
    # --- Step 3: Install/update Spotify if needed ---
    if (-not $installedVersion -or (-not $SkipInstall -and $installedVersion)) {
